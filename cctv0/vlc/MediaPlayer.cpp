...
void VlcMediaPlayer::open(VlcMedia *media, const int &time, bool paused)
{
    _media = media;
...
void VlcMediaPlayer::play(bool paused)
{
    m_paused = paused;
    if (!_vlcMediaPlayer)
        return;
                                     ...
                                     void *data)
{
    ...
    case libvlc_MediaPlayerPlaying:
        if ( core->m_paused )
            libvlc_media_player_set_pause(core->core(), true);
        else
            emit core->playing();
        break;
...
void VlcMediaPlayer::setRate(double rate)
{
    libvlc_media_player_set_rate(_vlcMediaPlayer, rate);
}
double VlcMediaPlayer::rate()
{
    return libvlc_media_player_get_rate(_vlcMediaPlayer);
}
void VlcMediaPlayer::takeSnapshot(const QString &filename, int width, int height)
{
    libvlc_video_take_snapshot(_vlcMediaPlayer, 0, filename.toAscii(), width, height);
}
