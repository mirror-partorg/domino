...
static int read_sk_sklad(do_alias_t *alias, sklad_key0_t *sklad_key0, struct tm *date1, struct tm *date2, const char *inn, do_string_list_t *text, int nds)
{
    ...
    for ( j = 0; j < list->count; j++ ) {
        ...
        for ( i = 0; i < check_info->lines->count; i++ ) {
            ...
            if ( !line->barcode ) {
                do_log(LOG_ERR, "product %s not found in sk list. Check %s/%s (Нет в текущих правилах)", do_text(alias, line->product.data.code), check_info->doc_num, do_text(alias, sklad_key0->code));
                return FALSE;
            }
            ...
            if ( long_report && !get_dist_inn(alias, sklad_key0, &check_info->time, check_info->cash, check_info->shift, &line->product, line->price9, &line->dist_inn, &line->nds) ) {
                do_log(LOG_ERR, "product %s not found in realization document. Check %s/%s от %s.Card %d", do_text(alias, line->product.data.code), check_info->doc_num, do_text(alias, sklad_key0->code), do_datetoa(check_info->time),check_info->cardno);
                return FALSE;
            }
            count++;
        }
    ...
    while ( status == DO_OK ) {
        ...
        if ( ((document.data.dtype1[0] == '0') && (document.data.dtype1[1] == '1' || document.data.dtype1[1] == '3') && (document.data.dtype1[2] == ' ')) &&
            ...
            while ( status == DO_OK ) {
                ...
                do_log(LOG_INFO,"%s.%s.%s.%d %s%s doc price: %.3f, check price: %.3f",
                do_text(alias, document_order.data.dtype),
                do_text(alias, document_order.data.sklad),
                do_text(alias, document_order.data.document),
                document_order.data.line,
                do_product_is_parcel(document_order.data.code, sizeof(document_order.data.code)) ? "" : "!Not parcel ",
                do_text(alias, document_order.data.code), price_9, price);
