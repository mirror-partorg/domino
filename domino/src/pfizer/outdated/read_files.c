...
static void rules_end (void *data, const char *el)
{
    ...
    switch (elem_type) {
        ...
        case ELEM_CARD_TYPE_CODE:
            ...
            break;
        case ELEM_CARD_NUMBER_FROM: {
            context->crnt_rule->start = atoll(s)-CARD_MASK;
            break;
        }
        case ELEM_CARD_NUMBER_TO:
            context->crnt_rule->end = atoll(s)-CARD_MASK;
            break;
        case ELEM_CARD_NUMBER: {
            context->crnt_rule->card = (atoll(s)-CARD_MASK*10);
            context->crnt_rule->card = ( context->crnt_rule->card - context->crnt_rule->card %10)/10;
            break;
        }
        case ELEM_SKU_RULE_ID:
        ...
        case ELEM_DATE_TO:
            ...
            break;
        case ELEM_DISCOUNT:
            context->crnt_line->discount = atoi(s);
            break;
        default:
            break;
    }
