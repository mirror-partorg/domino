...
static void
do_window_load_geometry (GtkWindow *window);
static void
do_window_save_geometry (GtkWindow *window);

static void
do_window_init (DoWindow *do_window)
{
	do_window->priv = G_TYPE_INSTANCE_GET_PRIVATE (do_window, DO_TYPE_WINDOW, DoWindowPrivate);
	do_window_load_geometry (GTK_WINDOW(do_window));
}

static void
do_window_finalize (GObject *object)
{
	do_window_save_geometry ( GTK_WINDOW (object));
...
GtkWidget *
do_window_new (const gchar *name)
{
