...
static void do_tree_view_cursor_popup_position_func (GtkMenu  *menu,
                     gint     *x,
                     gint     *y,
                     gboolean *push_in,
                     gpointer  data)
{
    DoTreeViewPrivate *priv = DO_TREE_VIEW_GET_PRIVATE(data);
    GtkTreeView *view = priv->tree_view;
    GtkWidget *w;
    GtkRequisition requisition;
    gint dx = 0, dy = 0;
    GdkRectangle cell_rect = {0,0,0,0};
    GtkTreePath *path = NULL;
    gint monitor_num;
    ...
    if ( path ) {
        gtk_tree_view_get_cell_area(GTK_TREE_VIEW(view), path, NULL, &cell_rect);
        gtk_tree_path_free(path);
    }
    gtk_tree_view_convert_bin_window_to_widget_coords(GTK_TREE_VIEW(view), cell_rect.x, cell_rect.y, &dx, &dy);
    *x += dx + 6;
    *y += dy + 6;
    ...
    if ( *x + requisition.width > monitor.width )
        *x -= requisition.width;
...
void do_tree_view_popup(DoTreeView *view, GdkEventButton* event, GtkWidget *menu)
{
    if ( event )
        ...
        gtk_menu_popup ( GTK_MENU( menu ), NULL, NULL,
                        do_tree_view_cursor_popup_position_func, view,
