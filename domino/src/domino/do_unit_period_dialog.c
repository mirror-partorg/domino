...
#define DO_UNIT_PERIOD_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), DO_TYPE_UNIT_PERIOD_DIALOG, DoUnitPeriodDialogPrivate))
#define DO_TYPE_UNIT_PERIOD_DIALOG_FLAGS do_unit_period_dialog_flags_get_type()


//#define OBJECT_ROOT_PATH "UnitPeriodDialog"
#define DEFAULT_WINDOW_POSITION "center-on-parent"
#define DEFAULT_WINDOW_SIZE ""
#define UNIT_PERIOD_TEMPLATE "unit_period"
...
struct _DoUnitPeriodDialogPrivate
{
...
G_DEFINE_TYPE (DoUnitPeriodDialog, do_unit_period_dialog, GTK_TYPE_DIALOG)

static GtkWidget *add_field(DoUnitPeriodDialog *dialog, gint id, GtkWidget *widget);

static void entry_activate(GtkWidget *entry, DoUnitPeriodDialog *dialog);
static void do_unit_period_dialog_read_last_value(DoUnitPeriodDialog *dialog);
static void do_unit_period_dialog_save_value(DoUnitPeriodDialog *dialog);
static gboolean do_unit_period_dialog_valid(DoUnitPeriodDialog *dialog);

static void do_unit_period_dialog_init(DoUnitPeriodDialog *dialog)
{

}
static void do_unit_period_dialog_ok(GtkButton *button, DoUnitPeriodDialog *dialog)
{
    if ( do_unit_period_dialog_valid(dialog) ) {
        do_unit_period_dialog_save_value(dialog);
        gtk_dialog_accept(GTK_DIALOG(dialog));
    }
}

static GObject *do_unit_period_dialog_constructor(GType type, guint n_construct_properties, GObjectConstructParam *construct_params)
{
	GObject *object;
	GtkWindow *window;
	GtkDialog *dialog;
	DoUnitPeriodDialog *unit_period_dialog;
	DoUnitPeriodDialogPrivate *priv;
	GtkWidget *vbox;
    GtkWidget *l;
    GtkWidget *t;
	...
	int row = 0;
	object = G_OBJECT_CLASS (do_unit_period_dialog_parent_class)->constructor(type, n_construct_properties, construct_params);

    priv = DO_UNIT_PERIOD_DIALOG_GET_PRIVATE(object);
	window = GTK_WINDOW (object);
	dialog = GTK_DIALOG (object);
	vbox   = gtk_dialog_get_content_area(dialog);
	unit_period_dialog = DO_UNIT_PERIOD_DIALOG(object);
	...
	gtk_window_set_title (window, priv->title);

    g_signal_connect (window, "configure-event",
                      G_CALLBACK (do_window_configure_event_cb), priv->name);
    ...
    l = add_field(unit_period_dialog, UNITS_ENTRY,
               do_unit_box_new());
    hig_workarea_add_row(t, &row, "Аптеки:", l, NULL);

	l = add_field(unit_period_dialog, DATE1_ENTRY, do_entry_date_box_new(DO_ENTRY_DATE_FLAGS_DEFAULT));
    hig_workarea_add_row(t, &row, "Начало:", l, NULL);

	l = add_field(unit_period_dialog, DATE2_ENTRY, do_entry_date_box_new(DO_ENTRY_DATE_FLAGS_DEFAULT));
    ...
    /*g_signal_connect (l, "clicked",
                      G_CALLBACK (do_unit_period_dialog_cancel), object);*/
	l = gtk_dialog_add_button (dialog, "Ok", GTK_RESPONSE_ACCEPT);
    g_signal_connect (l, "clicked",
                      G_CALLBACK (do_unit_period_dialog_ok), object);



	//gtk_dialog_set_default_response (dialog, GTK_RESPONSE_ACCEPT);
	do_unit_period_dialog_read_last_value(unit_period_dialog);

    DOMINO_PROFILE_OBJECT_INIT(G_OBJECT(window), priv->name,
...
static void do_unit_period_dialog_finalize (GObject *object)
{
	DoUnitPeriodDialogPrivate *priv = DO_UNIT_PERIOD_DIALOG_GET_PRIVATE (object);
	g_free(priv->name);
	g_free(priv->title);
	G_OBJECT_CLASS (do_unit_period_dialog_parent_class)->finalize (object);
}

static void do_unit_period_dialog_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	DoUnitPeriodDialogPrivate *priv = DO_UNIT_PERIOD_DIALOG_GET_PRIVATE (object);
...
static void do_unit_period_dialog_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	DoUnitPeriodDialogPrivate *priv = DO_UNIT_PERIOD_DIALOG_GET_PRIVATE (object);
    switch (prop_id)
    {
...
static void do_unit_period_dialog_class_init (DoUnitPeriodDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->constructor = do_unit_period_dialog_constructor;
	object_class->finalize = do_unit_period_dialog_finalize;
	object_class->get_property = do_unit_period_dialog_get_property;
	object_class->set_property = do_unit_period_dialog_set_property;

	g_type_class_add_private (object_class, sizeof (DoUnitPeriodDialogPrivate));
...
{
    GtkWidget *object;
    object = g_object_new (DO_TYPE_UNIT_PERIOD_DIALOG,
			     "main-window", parent,
			     "name", name,
			     "title", title,
			     (gpointer) NULL);
...
static void do_unit_period_dialog_read_last_value(DoUnitPeriodDialog *dialog)
{
    DoUnitPeriodDialogPrivate *priv = DO_UNIT_PERIOD_DIALOG_GET_PRIVATE(dialog);
    gchar *v_d_s_year;
    gchar *v_d_s_month;
    gchar *v_d_s_day;
    gchar *v_d_e_year;
...
static void entry_activate(GtkWidget *entry, DoUnitPeriodDialog *dialog)
{
    ...
    if ( valid )
        do_unit_period_dialog_ok(NULL, dialog);
}
...
static void do_unit_period_dialog_save_value(DoUnitPeriodDialog *dialog)
{
    ...
    DOMINO_PROFILE_SET(priv->name, "LastValue",
                                        "DateStartYear", v_d_s_year,
                                        "DateStartMonth", v_d_s_month,
                                        "DateStartDay", v_d_s_day,
                                        "DateEndYear", v_d_e_year,
