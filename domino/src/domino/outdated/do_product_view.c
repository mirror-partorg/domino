...
#define ALL_COLS_SHOW_WIDTH 800
...
#ifdef ADS_RECOMMEND
            GtkWidget *recom;
            recom = do_product_view_make_recommend(DO_PRODUCT_VIEW(object));
            //!!g_object_unref(view);
            gtk_notebook_append_page(GTK_NOTEBOOK(notebook), recom, NULL);
            gtk_widget_show_all(GTK_WIDGET(recom));
#endif
...
#ifdef ADS_RECOMMEND
                if ( !priv->search_char_count && priv->flags & DO_PRODUCT_VIEW_FLAGS_SHOW_RECOMMEND ) {
                    if ( priv->refill ) {
...
static void do_product_view_fill_recommend(DoProductView *view, const char *master, const char *select, const char *list_src)
{
	DoProductViewPrivate *priv = DO_PRODUCT_VIEW_GET_PRIVATE(view);
...
static gboolean do_product_view_recommend_key_press(GtkWidget *widget, GdkEventKey *event, DoProductView *do_view)
{
	...
	if ( (event->state & mask) == 0 )
	{
		switch (event->keyval)
		{
            ...
            default: {
                gunichar out;
                out = g_utf8_get_char(event->string);
                if ( g_unichar_isalpha(out) || g_unichar_isdigit(out) ) {
                    //!!do_tree_view_tree_view_set_visible(DO_TREE_VIEW(do_view), TRUE);
                    //gtk_widget_set_visible(GTK_WIDGET(priv->tree_view), TRUE);
                    //gtk_widget_set_visible(GTK_WIDGET(priv->recommend), FALSE);
                    gtk_notebook_set_current_page(GTK_NOTEBOOK(priv->notebook), 0);
...
void do_product_view_refill_recommend(DoProductView *view)
{
	DoProductViewPrivate *priv = DO_PRODUCT_VIEW_GET_PRIVATE(view);
	gtk_list_store_clear(priv->recommend_list);
	do_product_view_fill_recommend(view, "", "", NULL);
    GtkTreePath *path = NULL;
...
//#endif
    ...
    GtkWidget *scroll;
    priv->recommend_scroll = scroll = gtk_scrolled_window_new(NULL, NULL);
...
{
	DoProductViewPrivate *priv = DO_PRODUCT_VIEW_GET_PRIVATE(do_view);
    if ( gtk_widget_is_focus(GTK_WIDGET(priv->tree_view)) ) {
        gtk_widget_grab_focus(priv->recommend);
        gtk_notebook_set_current_page(GTK_NOTEBOOK(priv->notebook), 1);
