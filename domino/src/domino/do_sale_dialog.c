...
#define DO_SALE_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), DO_TYPE_SALE_DIALOG, DoSaleDialogPrivate))
#define DO_TYPE_SALE_DIALOG_FLAGS do_sale_dialog_flags_get_type()


#define OBJECT_ROOT_PATH "SaleDialog"
#define DEFAULT_WINDOW_POSITION "center-on-parent"
#define DEFAULT_WINDOW_SIZE ""
#define SALE_TEMPLATE "sale"
...
G_DEFINE_TYPE (DoSaleDialog, do_sale_dialog, GTK_TYPE_DIALOG)

static GtkWidget *add_field(DoSaleDialog *dialog, gint id, GtkWidget *widget);

static void entry_activate(GtkWidget *entry, DoSaleDialog *dialog);
static void do_sale_dialog_read_last_value(DoSaleDialog *dialog);
static void do_sale_dialog_save_value(DoSaleDialog *dialog);
static gboolean do_sale_dialog_valid(DoSaleDialog *dialog);

static void do_sale_dialog_init(DoSaleDialog *dialog)
{

}
static void do_sale_dialog_ok(GtkButton *button, DoSaleDialog *dialog)
{
    if ( do_sale_dialog_valid(dialog) ) {
        do_sale_dialog_save_value(dialog);
        gtk_dialog_accept(GTK_DIALOG(dialog));
    }
}

static GObject *do_sale_dialog_constructor(GType type, guint n_construct_properties, GObjectConstructParam *construct_params)
{
	GObject *object;
	GtkWindow *window;
	GtkDialog *dialog;
	DoSaleDialog *sale_dialog;
	//DoSaleDialogPrivate *priv;
	GtkWidget *vbox;
    GtkWidget *l;
    GtkWidget *t;
	...
	int row = 0;
	object = G_OBJECT_CLASS (do_sale_dialog_parent_class)->constructor(type, n_construct_properties, construct_params);

    //priv = DO_SALE_DIALOG_GET_PRIVATE(object);
	window = GTK_WINDOW (object);
	dialog = GTK_DIALOG (object);
	vbox   = gtk_dialog_get_content_area(dialog);
	sale_dialog = DO_SALE_DIALOG(object);
	...
	gtk_window_set_title (window, "Выручка за период");
    ...
    l = add_field(sale_dialog, UNITS_ENTRY,
               do_unit_box_new());
    hig_workarea_add_row(t, &row, "Аптеки:", l, NULL);

	l = add_field(sale_dialog, DATE1_ENTRY, do_entry_date_box_new(DO_ENTRY_DATE_FLAGS_DEFAULT));
    hig_workarea_add_row(t, &row, "Начало:", l, NULL);

	l = add_field(sale_dialog, DATE2_ENTRY, do_entry_date_box_new(DO_ENTRY_DATE_FLAGS_DEFAULT));
    ...
    /*g_signal_connect (l, "clicked",
                      G_CALLBACK (do_sale_dialog_cancel), object);*/
	l = gtk_dialog_add_button (dialog, "Ok", GTK_RESPONSE_ACCEPT);
    g_signal_connect (l, "clicked",
                      G_CALLBACK (do_sale_dialog_ok), object);



	//gtk_dialog_set_default_response (dialog, GTK_RESPONSE_ACCEPT);
	do_sale_dialog_read_last_value(sale_dialog);
...
static void do_sale_dialog_finalize (GObject *object)
{
//	DoSaleDialogPrivate *priv = DO_SALE_DIALOG_GET_PRIVATE (object);
	G_OBJECT_CLASS (do_sale_dialog_parent_class)->finalize (object);
}

static void do_sale_dialog_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	DoSaleDialogPrivate *priv = DO_SALE_DIALOG_GET_PRIVATE (object);
...
static void do_sale_dialog_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	DoSaleDialogPrivate *priv = DO_SALE_DIALOG_GET_PRIVATE (object);
    switch (prop_id)
    {
...
static void do_sale_dialog_class_init (DoSaleDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->constructor = do_sale_dialog_constructor;
	object_class->finalize = do_sale_dialog_finalize;
	object_class->get_property = do_sale_dialog_get_property;
	object_class->set_property = do_sale_dialog_set_property;

	g_type_class_add_private (object_class, sizeof (DoSaleDialogPrivate));
...
{
    GtkWidget *object;
    object = g_object_new (DO_TYPE_SALE_DIALOG,
			     "main-window", parent,
			     (gpointer) NULL);
    return object;
}
...
static void do_sale_dialog_read_last_value(DoSaleDialog *dialog)
{
    DoSaleDialogPrivate *priv = DO_SALE_DIALOG_GET_PRIVATE(dialog);
    gchar *v_d_s_year;
    gchar *v_d_s_month;
    gchar *v_d_s_day;
    gchar *v_d_e_year;
