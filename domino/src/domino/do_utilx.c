...
gboolean do_domino_auth(DoPassword *password, gpointer data)
{
    ...
    iconv_t *conv;
    gchar *name, *passwd;
    char *domino_name, *domino_passwd;
    gboolean retval;

    conv = iconv_open(DOMINO_CHARSET, UTF_CHARSET);
    if (conv == (iconv_t)-1) {
        DOMINO_SHOW_INFO("Ошибка выполнения процедуры авторизации");
        do_log(LOG_ERR, "Error iconv_open (\"%s\" to \"%s\") ", UTF_CHARSET, DOMINO_CHARSET);
        return FALSE;
    }

    g_object_get(password, "name", &name, "password", &passwd, NULL);

    domino_name = iconv_(conv, name);
    domino_passwd = iconv_(conv, passwd);
    switch (domino_auth(domino_name, domino_passwd)) {
        case -1:
            DOMINO_SHOW_INFO("Ошибка доступа к ресурсу авторизации");
            retval = FALSE;
            break;
        case 0:
            DOMINO_SHOW_INFO("Имя пользователя или пароль неправильны");
            retval = FALSE;
            break;
        default:
            retval = TRUE;
...
void do_window_save_setting(GtkWindow *window, const gchar *path)
{
    ...
    if (!(state &
        ...
        if (DO_IS_WINDOW(window))
            DOMINO_PROFILE_SET(path, "DoWindow", "window-size", buffer, NULL)
        else
...
/*            case GTK_WIN_POS_CENTER:
        ...
        if (buffer[0]) {
            if (DO_IS_WINDOW(window))
                DOMINO_PROFILE_SET(path, "DoWindow", "window-position", buffer, NULL)
            else
...
gboolean do_key_press_clipboard_event (GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    ...
    if ((event->state & mask) == GDK_CONTROL_MASK && event->length) {
...
gboolean do_key_press_accel_event (GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    guint mask = gtk_accelerator_get_default_mod_mask();
    if ((event->state & mask) == GDK_CONTROL_MASK && event->length) {
        if (gtk_accel_groups_activate(G_OBJECT (widget), event->hardware_keycode, event->state))
            return TRUE;
    }
    return FALSE;
}
...
static void grid_changed(GtkComboBox *widget, gpointer     user_data)
{
    gint lines;
    lines = gtk_combo_box_get_active(widget);
    do_windows_set_tree_view_lines(DO_WINDOW(domino_get_main_window()), lines);
}
#endif
...
gboolean do_common_edit(GtkWidget *widget)
{
    ...
    if ( alias ) {
        gtk_entry_set_text(GTK_ENTRY(local_entry), do_alias_get_host(alias));
        do_alias_free(alias);
    }
    ...
    if ( alias ) {
        gtk_entry_set_text(GTK_ENTRY(main_entry), do_alias_get_host(alias));
        do_alias_free(alias);
    }
...
#ifdef _WIN32
        if ( domino_configure(gtk_entry_get_text(GTK_ENTRY(local_entry)),
                             gtk_entry_get_text(GTK_ENTRY(main_entry)),
                              gtk_entry_get_text(GTK_ENTRY(unit_entry))) ) {
            retval = TRUE;
            break;
        }
	else
...
gboolean do_alias_text_in_list(do_alias_t *alias, void *src, size_t size , ...)
{
    va_list args;
    gchar *value;
    gchar *name;
    value = do_alias_text_(alias, src, size);
    va_start (args, size);
...
gboolean get_purchase(do_alias_t *alias, product_rec_t *product, const char *unit, purchase_t *last)
{
    ...
    value = g_strdup_printf("%s.%s", unit, code);
    ...
    for (i = 0; i < 4 && (status == DO_OK); i++) {
        ...
        if ( strlen(buf) < 149 )
            ...
            last[i].name[150] = '\0';
        do_free(buf);
        for ( ch = last[i].name; *ch != '\0'; ch++ )
            if ( *ch == ' ' ) {
                *ch = '\0';
                if ( !strcmp(last[i].name, "АПТЕКА") ) {
                    char *head;
                    ch++;
                    head=ch;
                    gunichar out;
                    for (; ch && *ch !='\0'; ch = (gchar*)g_utf8_next_char(ch)) {
                        out = g_utf8_get_char(ch);
