...
int dominox_init(const char *filename)
{
    if ( !domino_init(NULL, TRUE, DO_PARAM_END) ) {
        DOMINO_SHOW_ERROR("Окружение домино не инициализировано");
        return FALSE;
    }
    do_gtk_log_init();
    gint i;
    ...
    for (i = 0; i < DOMINO_CONFIG_N; i++)
        if ( !domino_config_read(i) )
            return FALSE;
    g_set_application_name(DOMINO_APPLICATION_NAME);
#ifdef _WIN32
    if ( !domino_check_configured() ) {
...
gchar *domino_get_default_location_name()
{
    ...
    if ( !alias )
        return strdup("");
    if ( !do_alias_open(alias, FALSE, DO_DB_SKLAD, DO_DB_PARTNER, DO_DB_OTDEL, DO_DB_END) ) {
        do_alias_free(alias);
        return strdup("");
    }
    otdel_rec_t otdel;
    ...
    switch ( do_otdel_get0(alias, &otdel, &otdel_key0, DO_GET_EQUAL) ) {
        ...
        case DO_KEY_NOT_FOUND:
            do_log(LOG_ERR, "otdel \"%s\" not found", domino_division());
        default:
            do_alias_free(alias);
            return strdup("");
    }
    value = do_otdel_param(alias, &otdel, do_param(DO_PARAM_OTDEL_PARAMETR_FIRM));
    if ( !do_util_str_to_partner_key0(alias, value, &partner_key0) ) {
        do_log(LOG_ERR, "firm not set");
        do_alias_free(alias);
        do_free(value);
        return strdup("");
    }
    ...
    if ( *unit != '\0' ) {
        ...
        sklad_name = do_text(alias, sklad.data.name);
    }

    if ( sklad_name )
        value = g_strdup_printf("%s (%s)", firm, sklad_name);
    else
        value = g_strdup_printf("%s", firm);
    do_free(firm);
    if (sklad_name) do_free(sklad_name);
    do_alias_free(alias);
    return (value);
}
...
void domino_update_main_window_title()
{
    ...
    //gtk_header_bar_set_title (GTK_HEADER_BAR(gtk_window_get_titlebar(GTK_WINDOW (main_window))), title)
#endif
...
gboolean domino_get_ui(gchar **ui, gsize *length)
{
    ...
    char *ui_name = NULL;
    int i;
    groups = domino_user_groups();

    for (i = 0; i < groups->count; i++) {
        char *group = groups->list[i];
        char *ch = strchr (group, '.');
        if (ch && *(ch+1) == 'u' && *(ch+2) == 'i' && *(ch+3) == '\0') {
            ui_name = group;
            break;
        }
    }
    if (!ui_name)
        ui_name = DOMINO_DEFAULT_UI;

    char *filename;

    filename=do_share_get_ui_filename(ui_name);
    if ( !filename ) {
        do_log(LOG_ERR, "Интерфейс пользователя не найден");
        return FALSE;
    }

    if ( g_file_get_contents (filename, ui, length, NULL) ) {
        g_free(filename);
        return TRUE;
    }
    g_free(filename);
...
{
    if (!(domino_get_role() & roles)) {
        DOMINO_SHOW_INFO("Для выполнения операции недостаточно прав");
        return FALSE;
    }
    return TRUE;
}
