...
static gboolean	command_can_close_for_esc(DoView *view);
...
G_DEFINE_TYPE_WITH_CODE (DoHtmlView, do_html_view, GTK_TYPE_BOX,
             ...
             G_ADD_PRIVATE(DoHtmlView)
                            );

static void do_html_view_view_init(DoViewIface *iface)
{
    iface->do_close        = view_close;
    iface->can_do_close_for_esc = command_can_close_for_esc;
                                                        ...
                                                        DoView *view)
{
	...
	if ( win && DO_IS_WINDOW(win) )
        do_window_set_toolbar_progress(DO_WINDOW(win), 0);
                                                        ...
                                                        DoView *view)
{
	...
	GtkWidget *win = gtk_widget_get_toplevel(GTK_WIDGET(view));
	if ( win && DO_IS_WINDOW(win) )
        do_window_set_toolbar_progress(DO_WINDOW(win), priv->load_progress);

}
static WebKitWebView *
do_html_view_create (WebKitWebView          *web_view,
...
#ifdef WEBKIT2
    g_signal_connect(priv->view_html,  "load-changed",  G_CALLBACK(do_html_view_load_changed), object);
    g_signal_connect(priv->view_html,  "create",  G_CALLBACK(do_html_view_create), object);
#else
...
static void do_set_html_string(DoHtmlView *view, const gchar *string)
{
    ...
    //fix me webkit_web_view_load_html_string(WEBKIT_WEB_VIEW(priv->view_html),
