...
def make_method(ClassName,structName,st):
	...
	res=""
	get_str="""
static PyObject *%s_get_%s(Alias* self)
{
    PyObject *result;
"""
	for l in st.split("\n"):
		...
		name=dd.group("name")
		tt=dd.group("type")
		res = res + get_str%(ClassName,name)
		if tt="BTI_BYTE":
			if size == 1:
				res = res + """result = Py_BuildValue("i", self->priv->%s));"""%(name)
			else
				res = res + """result = Py_BuildValue( self->priv->%s));"""%(name)
		elif tt="BTI_CHAR":
			res = res + """
char *res=do_text( result = Py_BuildValue("i", self->priv->%s));"""%(name)
