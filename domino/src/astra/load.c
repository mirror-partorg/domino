...
int main(int argc, char *argv[])
{
    ...
    if ( argc < 3 ) {
        fprintf(stderr, "usage %s <filename> [+tuning_file] <alias> [<alias>..] %d\n", argv[0],PFIZER_SK);
        exit(1);
    }
    filename = argv[1];
    ...
    if ( argv[2][0] == '+' ) {
        do_log(LOG_INFO, "load tunning");
        if ( !pfizer_rules_add_from_file(rules, argv[2]+1) )
            exit(1);
        startc ++;
    }
...
static int insert_sk(do_alias_t *alias, pfizer_rule_t *rule)
{
    ...
    do_document_param_int_set(alias, &document, "OFF",rule->one_off);
...
static int insert_sk_line(do_alias_t *alias, pfizer_rule_t *rule, pfizer_line_t *line)
{
    ...
    do_text_set(alias, document_order_key0.dtype, do_param(DO_PARAM_DOCUMENT_TYPE_DISCOUNT));
    ...
    document_order_key0.line=line->id;
    status = do_document_order_key0(alias, &document_order_key0, DO_GET_EQUAL);
#ifdef NO_CLEAR
    if ( status == DO_OK ) 
        return TRUE;
#endif
    ...
    //do_log(LOG_INFO ,"line max %s %f", line->desc, line->maxdiscount);
    if ( line->quant > 0 )
        do_document_order_quant_set(alias, &document_order, 1, line->quant*coef);
    if ( line->maxquant > 0 )
...
static int load_stop_list(const char *alias_name, pfizer_rules_t *rules)
{
    ...
    for ( i = 0; retval && i < rules->rules->count; i++ ) {
        ...
        partner.data.code = rule->card;//+1000000000;
        partner.data.type = 7;
        do_text_set(alias, partner.data.name, "ЯЯЯ Astra card block");
