...
static int read_sk_sklad(do_alias_t *alias, sklad_key0_t *sklad_key0, struct tm *date1, struct tm *date2, const char *inn, do_string_list_t *text, int nds)
{
    ...
    while ( status == DO_OK ) {
        ...
        if ( (card > PFIZER_SKB) && (card < PFIZER_SKB_MAX) && (do_document_param(alias, &document, "CONFIRM")[0] != '0')) {
            char *val, *ch;
...
static char *get_barcode(do_alias_t *alias, sklad_key0_t *sklad_key0, int cardno, product_rec_t *product, const char *in_barcode)
{
    ...
    while ( status == DO_OK ) {
        ...
        if ( document_key3.date <= cardno && document_key3.time >= cardno ) {
            ...
            while ( status  == DO_OK ) {
                ...
                fprintf(stderr, "barcode %s %s %s %s %s %d %s %s.%s.%s\n", do_text(alias,product->data.code),do_document_order_param(alias, &document_order, "BAR"),do_text(alias,document_order.data.document),do_text(alias,document_order.data.dtype),do_text(alias,document.data.sklad),document_order.data.line, in_barcode,do_text(alias,document.data.dtype),do_text(alias,document.data.sklad),do_text(alias,document.data.document));
                if ( !strlen(res) && in_barcode[0] != '2' ) {
                    ...
                    do_free(res);
                    res = do_strdup(in_barcode);
                    fprintf(stderr, "barcode %s %s %s %s %s %d %s\n", do_text(alias,product->data.code),res,do_text(alias,document_order.data.document),do_text(alias,document_order.data.dtype),do_text(alias,document.data.sklad),document_order.data.line, in_barcode);
                }
                if ( !strlen(res) ) {
                    do_free(res);
                    res = NULL;
                    ...
                    char *str_=do_text(alias, document_order.data.code);
                    str_[5]='\0';
                    do_text_set(alias,barcode_key1.code,str_);
                    do_free(str_);
                    status = do_barcode_get1(alias, &barcode, &barcode_key1, DO_GET_GE);
                    while ( status == DO_OK ) {
                        if ( do_cmp(barcode_key1.code,document_order.data.code) ) break;
                        res = do_text(alias, barcode.data.barcode);
                        if ( res[1] != '2' ) break;
                        do_free(res);
                        res = NULL;
    ...
    if ( !res || !strlen(res) ) {
        fprintf(stderr, "no barcode for %s\n", do_text(alias,product->data.code));
        return NULL;
    }
    ...
    while ( status == DO_OK ) {
        ...
        if ( ((document.data.dtype1[0] == '0') && (document.data.dtype1[1] == '1' || document.data.dtype1[1] == '3') && (document.data.dtype1[2] == ' ')) &&
            ...
            while ( status == DO_OK ) {
                ...
                do_log(LOG_INFO,"%s doc price: %.3f, check price: %.3f",do_text(alias, document_order.data.code), price_9, price);
