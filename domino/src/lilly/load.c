...
int main(int argc, char *argv[])
{
    ...
    if ( argc < 3 ) {
        fprintf(stderr, "usage %s <filename> <alias> [<alias>..] %d\n", argv[0],PFIZER_SK);
        exit(1);
    }
    filename = argv[1];
...
static int clear_rules(do_alias_t *alias)
{
    ...
    do_log(LOG_INFO, "clear %s.%s.%s-%s",do_text(alias,document_key0.dtype),do_text(alias,document_key0.sklad),do_text(alias,document_key0.document),do_text(alias,key.document));
    ...
    while ( status == DO_OK ) {
        ...
        do_log(LOG_INFO, "delete doc %s.%s.%s",do_text(alias,document_key0.dtype),do_text(alias,document_key0.sklad),do_text(alias,document_key0.document));        
        ...
        while ( status == DO_OK ) {
            ...
            do_log(LOG_INFO, "delete doc line %s.%s.%s.%d",do_text(alias,document_key0.dtype),do_text(alias,document_key0.sklad),do_text(alias,document_key0.document),document_order_key0.line);
...
static int insert_sk(do_alias_t *alias, pfizer_rule_t *rule)
{
    ...
    /*rule_id_inc+=1;
    sprintf(buf,"%d", PFIZER_SK+rule_id_inc);*/
...
static int load_stop_list(const char *alias_name, pfizer_rules_t *rules)
{
    ...
    for ( i = 0; retval && i < rules->rules->count; i++ ) {
        ...
        partner.data.code = rule->card;//+2000000000;
        partner.data.type = 7;
        do_text_set(alias, partner.data.name, "ЯЯЯ lilly card block");
...
static int clear_stop_list(do_alias_t *alias)
{
    ...
    while ( status == DO_OK ) {
        do_log(LOG_INFO, "crnt %d stop %d",partner_key0.code,key.code);
