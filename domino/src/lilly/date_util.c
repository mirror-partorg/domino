...
int mk_date(const char *s, struct tm *tm)
{
    ...
    day_of_the_week(tm);
    day_of_the_year(tm);
    if (!(tm->tm_year > -70 && tm->tm_year < 10000) && (tm->tm_mon >=0 && tm->tm_mon <12) &&
              (tm->tm_mday > 0 && tm->tm_mday < 32)) {
       do_log(LOG_INFO,"not valid date %d %d %d", (tm->tm_year > -70 && tm->tm_year < 10000), (tm->tm_mon >=0 && tm->tm_mon <12),
              (tm->tm_mday > 0 && tm->tm_mday < 32));
    }
