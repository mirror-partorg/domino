...
#endif
{
    ...
    Py_INCREF(getRealizationType());

    PyModule_AddObject(m, "Realization", (PyObject *)getRealizationType());

    if ( PyType_Ready(getRealizationKey0Type()) < 0 )
         goto init_error;

    Py_INCREF(getRealizationKey0Type());

    PyModule_AddObject(m, "RealizationKey0", (PyObject *)getRealizationKey0Type());

    if ( PyType_Ready(getRealizationKey1Type()) < 0 )
         goto init_error;

    Py_INCREF(getRealizationKey1Type());

    PyModule_AddObject(m, "RealizationKey1", (PyObject *)getRealizationKey1Type());

    if ( PyType_Ready(getRealizationKey2Type()) < 0 )
         goto init_error;

    Py_INCREF(getRealizationKey2Type());

    PyModule_AddObject(m, "RealizationKey2", (PyObject *)getRealizationKey2Type());

    if ( PyType_Ready(getRealizationKey3Type()) < 0 )
         goto init_error;

    Py_INCREF(getRealizationKey3Type());

    PyModule_AddObject(m, "RealizationKey3", (PyObject *)getRealizationKey3Type());

    if ( PyType_Ready(getRealizationKey4Type()) < 0 )
         goto init_error;

    Py_INCREF(getRealizationKey4Type());
    ...
    Py_INCREF(getCheckType());

    PyModule_AddObject(m, "Check", (PyObject *)getCheckType());

    if ( PyType_Ready(getCheckKey0Type()) < 0 )
         goto init_error;

    Py_INCREF(getCheckKey0Type());

    PyModule_AddObject(m, "CheckKey0", (PyObject *)getCheckKey0Type());

    if ( PyType_Ready(getCheckKey1Type()) < 0 )
         goto init_error;

    Py_INCREF(getCheckKey1Type());

    PyModule_AddObject(m, "CheckKey1", (PyObject *)getCheckKey1Type());

    if ( PyType_Ready(getCheckKey2Type()) < 0 )
         goto init_error;

    Py_INCREF(getCheckKey2Type());

    PyModule_AddObject(m, "CheckKey2", (PyObject *)getCheckKey2Type());
