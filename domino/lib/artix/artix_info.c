...
DO_EXPORT int artix_info_get_module_description(artix_info_t *info, const char *module_name, char*** module_desc, int *count_desc)
{
    ...
    int psize = sizeof(int8_t) + sizeof(int16_t) + sizeof(int32_t) + strlen(module_name);
    char params[psize];
    char *cp = params;
    *cp = T_STRING;
    cp++;
    ...
    memcpy(cp, &id, sizeof(id));
    cp += sizeof(id);
    int32_t value = (int32_t)ntohl(strlen(module_name));
    memcpy(cp, &value, sizeof(value));
    cp += sizeof(value);
    memcpy(cp, module_name, strlen(module_name));
    cp += strlen(module_name);

    int32_t cseqid = 0;
    if (!thrift_send_function(info->sock, GETMODULEDESCRIPTION, cseqid, params, psize))
        return 0;
    ...
    while (1) {
        ...
        switch (field_id) {
            case 0: {
                ...
                *count_desc = size;
                *module_desc = (char**) do_malloc (size*sizeof(char*));
                for (i = 0; i < size; i++) {
                    if (!thrift_read_str(info->sock, &buf))
                        return 0;
                    (*module_desc)[i] = buf;
